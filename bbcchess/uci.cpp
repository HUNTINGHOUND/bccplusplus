...
Move parse_move(std::string const & move_string, BoardRepresentation const & rep, size_t move_idx = 0) {
    ...
    BitBoardSquare source_square = static_cast<BitBoardSquare>((move_string[move_idx] - 'a') + (8 - (move_string[move_idx + 1] - '0')) * 8);
    BitBoardSquare target_square = static_cast<BitBoardSquare>((move_string[move_idx + 2] - 'a') + (8 - (move_string[move_idx + 3] - '0')) * 8);
    
    for (int move_count = 0; move_count < move_list.count; move_count++) {
        Move& move = move_list.moves[move_count];
        
        if (source_square == move.get_move_source() && target_square == move.get_move_target()) {
            BoardPiece::Pieces promoted_piece = static_cast<BoardPiece::Pieces>(move.get_move_promoted());
...
void parse_position(std::string const & command, BoardRepresentation & rep) {
    ...
    current_char = command.find("moves", current_char);
    ...
    if (current_char != std::string::npos) {
        ...
        std::cout << (current_char < command.size() ? command.substr(current_char) : "") << "\n";
    }
}
